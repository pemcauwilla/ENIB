...
/* --------------- MEMORY POOL -----------------*/
volatile uint32_t msgs_sent = 0;
volatile uint32_t msgs_processed = 0;
/* ---------------------------------------------*/
...
/* ------------------- MEMORY POOL -------------------*/
#define SHMEM_MAX_WORDS 4096

SPoint *shared_pool = __start_noinit_shmem;
uint32_t current_offset = 0;

SPoint* alloc_shmem_buffer(int n, uint32_t *offset) {
    if (current_offset + n > SHMEM_MAX_WORDS) {
        while (msgs_sent > msgs_processed) { // Sleep while there's not enough space to be liberated
            __WFI();
        }

        current_offset = 0; // Reset pool
    }

    *offset = current_offset;
    SPoint* allocated_ptr = &shared_pool[current_offset];
    current_offset += n;
...
{
    uint32_t offset;
    SPoint *buf = alloc_shmem_buffer(1, &offset);

    buf[0].x = (short) font->width;
    buf[0].y = (short) font->height;

    // Offset (bits 11 to 23) - Font's first byte (bits 0 to 7)
    uint32_t msg = EVT_SETFONT | ((offset & 0x1FFF) << 11) | ((*font->data) & 0xFF);

    while (!mb_push_evt(msg, false)) {
    	__WFI();
    }
    msgs_sent++;
...
{
    // 16 lower bits -> color
    uint32_t msg = EVT_FORECOLOR | (uint16_t)color;
...
uint32_t lcd_set_alignment(DC *dc, uint32_t alignment)
{
    ...
    buf[0].x = (short)(alignment & 0xFFFF);
    buf[0].y = (short)((alignment >> 16) & 0xFFFF);

    uint32_t msg = EVT_SETALIGN | ((offset & 0x1FFF) << 11);
    while (!mb_push_evt(msg, false)) {
    	__WFI();
    }
    msgs_sent++;
...
uint32_t lcd_set_direction(DC *dc, uint32_t direction)
{
    ...
    buf[0].x = direction & 0xFFFF;
    buf[0].y = (direction >> 16) & 0xFFFF;

    uint32_t msg = EVT_SETDIR | ((offset & 0x1FFF) << 11);
    while (!mb_push_evt(msg, false)) {
    	__WFI();
    }
    msgs_sent++;
...
{
    uint32_t offset;
    SPoint *buf = alloc_shmem_buffer(3, &offset);

    buf[0].x = (short)x; buf[0].y = (short)y;
    buf[1].x = (short)w; buf[1].y = (short)h;
    buf[2].x = (short)c;

    uint32_t msg = EVT_DRAWRECT | ((offset & 0x1FFF) << 11);
    while (!mb_push_evt(msg, false)) {
    	__WFI();
    }
    msgs_sent++;
...
{
	// Calculate offset to send to core 1
	uint32_t offset = p - shared_pool;

    p[2 * n].x = (short) c;

    uint32_t msg = (EVT_DRAWSEGMENTS) | ((offset & 0x1FFF) << 11) | (n & 0x7FF);
...
void lcd_draw_lines(SPoint *p, int n)
{
    if (n < MAXPOINTS) {
        ...
        uint32_t msg = (EVT_DRAWLINES) | ((offset & 0x1FFF) << 11) | (n & 0x7FF);
...
void lcd_draw_path2d(Graph *g, SPoint *p, int n)
{
    if (n < MAXPOINTS) {
        ...
        p[n+1].x = (short) g->msize;
        p[n+1].y = (short) g->pyorg;

        p[n+2].x = (short) g->pxmin;
        p[n+2].y = (short) g->pxmax;

        p[n+3].x = (short) g->pymin;
        p[n+3].y = (short) g->pymax;

        p[n+4].x = (short) g->color;

        uint32_t msg = EVT_DRAWPATH | ((offset & 0x1FFF) << 11) | (n & 0x7FF);
...
void lcd_fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    uint32_t msg = EVT_FILLRECT | ((offset & 0x1FFF) << 11);
...
{
    uint32_t offset;
    SPoint *buf = alloc_shmem_buffer(2, &offset);

    buf[0].x = (short)x1; buf[0].y = (short)y1;
    buf[1].x = (short)x2; buf[1].y = (short)y2;

    uint32_t msg = EVT_CLIP | ((offset & 0x1FFF) << 11);
    while (!mb_push_evt(msg, false)) {
    	__WFI();
    }
    msgs_sent++;
...
void lcd_draw_string(DC *dc, int16_t x, int16_t y, const char *s)
{
    ...
    // Caculate how many bytes are needed (max 100)
    int len = strlen(s);
    if (len > 100) len = 100;

    // 1 char = 1 byte. 1 SPoint = 4 bytes.
    // +1 to store x and y positions
    int n_blocos = 1 + ((len + 3) / 4); // +3 just to round up and not loose data

    SPoint *buf = alloc_shmem_buffer(n_blocos, &offset);

    buf[0].x = x;
    buf[0].y = y;

    char *destination = (char *)&buf[1];
    strncpy(destination, s, len);
    destination[len] = '\0';

    // Send offset (11 to 23) and length (0 to 10)
    uint32_t msg = EVT_DRAWSTRING | ((offset & 0x1FFF) << 11) | (len & 0x7FF);

    while (!mb_push_evt(msg, false)) { __WFI(); }
    msgs_sent++;
}
...
void g_xgrid(Graph *g, header *xticks, real factor, unsigned int color)
{
    ...
    if (!(g->flags & G_XLOG)) {
        ...
        // Draw grid
        if (g->flags & G_XGRID) {
            // Store new buffer ONLY for grid
        	buf = alloc_shmem_buffer(2 * n + 1, &offset);
            for (int i=0 ; i<n ; i++) {
                buf[2*i].x = (short)(g->pxmin + (ticks[i] - g->xmin) * g->xfactor);
                buf[2*i].y = g->pymin;
                buf[2*i+1].x = buf[2*i].x;
                buf[2*i+1].y = g->pymax;
            }
            lcd_draw_segments(buf, n, gridcolor);
        }

        // Draw lower ticks - another separate buffer ONLY FOR LOWER TICKS
        buf = alloc_shmem_buffer(2 * n + 1, &offset);
        for (int i=0 ; i<n ; i++) {
            buf[2*i].x = (short)(g->pxmin + (ticks[i] - g->xmin) * g->xfactor);
            buf[2*i].y = g->pymin;
            buf[2*i+1].x = buf[2*i].x;
            buf[2*i+1].y = g->pymin + TICKSIZE;
        }
        lcd_draw_segments(buf, n, BLACK);

        // Upper ticks and texts - same logic, NEW BUFFER for upper ticks and texts
        buf = alloc_shmem_buffer(2 * n + 1, &offset);
        for (int i=0 ; i<n ; i++) {
            short px = (short)(g->pxmin + (ticks[i] - g->xmin) * g->xfactor);
            buf[2*i].x = px;
            buf[2*i].y = g->pymax - TICKSIZE;
            buf[2*i+1].x = px;
            buf[2*i+1].y = g->pymax;

            char s[32];
            snprintf(s,32,"%g",fabs(ticks[i]/factor) < 1e-6 ? 0.0 : ticks[i]/factor);
            lcd_draw_string(&dc, px, g->pymax + fixed12.height/2, s);
        }
        lcd_draw_segments(buf, n, BLACK);
        ...
        if (g->flags & G_XGRID) {
            buf = alloc_shmem_buffer(2 * n + 1, &offset);
            for (int i=0 ; i<n ; i++) {
                buf[2*i].x = (short)(g->pxmin + g->xfactor*log10(ticks[i]/g->xmin));
                buf[2*i].y = g->pymin;
                buf[2*i+1].x = buf[2*i].x;
                buf[2*i+1].y = g->pymax;
            }
        ...
        buf = alloc_shmem_buffer(2 * n + 1, &offset);
        for (int i=0 ; i<n ; i++) {
            short px = (short)(g->pxmin + g->xfactor*log10(ticks[i]/g->xmin));
            real d = ticks[i]/pow(10.0,floor(log10(ticks[i])));

            buf[2*i].x = px;
            buf[2*i].y = g->pymin;
            buf[2*i+1].x = px;
            buf[2*i+1].y = g->pymin + (d!=1.0 ? SUBTICKSIZE : TICKSIZE);
        }
        lcd_draw_segments(buf, n, BLACK);

        // Upper ticks and texts (LOG)
        buf = alloc_shmem_buffer(2 * n + 1, &offset);
        for (int i=0 ; i<n ; i++) {
            short px = (short)(g->pxmin + g->xfactor*log10(ticks[i]/g->xmin));
            ...
            buf[2*i+1].y = g->pymax;
            buf[2*i].x = px;
            buf[2*i].y = g->pymax - (d!=1.0 ? SUBTICKSIZE : TICKSIZE);
            ...
            if (d==1.0) {
                ...
                lcd_set_alignment(&dc,ALIGN_S);
                lcd_draw_string(&dc,px,g->pymax+3*fixed12.height/4,"10");
                lcd_set_font(&dc,&fixed8);
                lcd_set_alignment(&dc,ALIGN_E);
                lcd_draw_string(&dc,px+fixed12.width,g->pymax+3*fixed12.height/4,s);
            }
        }
        lcd_draw_segments(buf, n, BLACK);
    }
...
void g_ygrid(Graph *g, header *yticks, real factor, unsigned int color)
{
    ...
    if (!(g->flags & G_YLOG)) {
        ...
        if (g->flags & G_YGRID) {
            ...
            for (int i=0 ; i<n ; i++) {
                buf[2*i].x = g->pxmin;
                buf[2*i].y = g->pymax - g->yfactor*(ticks[i]-g->ymin);
                buf[2*i+1].x = g->pxmax;
                buf[2*i+1].y = buf[2*i].y;
            }
            lcd_draw_segments(buf, n, gridcolor);
        }

        // LEFT TICKS AND TEXTS
        buf = alloc_shmem_buffer(2 * n + 1, &offset);
        for (int i=0 ; i<n ; i++) {
            short py = g->pymax - g->yfactor*(ticks[i]-g->ymin);
            buf[2*i].x = g->pxmin;
            buf[2*i].y = py;
            buf[2*i+1].x = g->pxmin + TICKSIZE;
            buf[2*i+1].y = py;

            char s[32];
            snprintf(s,32,"%g",fabs(ticks[i]/factor) < 1e-6 ? 0.0 : ticks[i]/factor);
            lcd_draw_string(&dc, g->pxmin-fixed12.width/2, py, s);
        }
        lcd_draw_segments(buf, n, BLACK);
        ...
        if (g->flags & G_YGRID) {
            ...
            for (int i=0 ; i<n ; i++) {
                buf[2*i].x = g->pxmin;
                buf[2*i].y = g->pymax - g->yfactor*log10(ticks[i]/g->ymin);
                buf[2*i+1].x = g->pxmax;
                buf[2*i+1].y = buf[2*i].y;
            }
            lcd_draw_segments(buf, n, gridcolor);
        ...
        // LEFT TICKS AND TEXTS (LOG)
        buf = alloc_shmem_buffer(2 * n + 1, &offset);
        for (int i=0 ; i<n ; i++) {
            short py = g->pymax - g->yfactor*log10(ticks[i]/g->ymin);
            real e = floor(log10(ticks[i]));
            real d = ticks[i]/pow(10.0,e);

            buf[2*i].x = g->pxmin;
            buf[2*i].y = py;
            buf[2*i+1].x = g->pxmin + (d!=1.0 ? SUBTICKSIZE : TICKSIZE);
            buf[2*i+1].y = py;
            ...
            if (d==1.0) {
                ...
                lcd_set_font(&dc,&fixed12);
                lcd_set_alignment(&dc,ALIGN_W);
                lcd_draw_string(&dc, g->pxmin-fixed12.width/2-len*fixed8.width, py, "10");
                lcd_set_font(&dc,&fixed8);
                lcd_set_alignment(&dc,ALIGN_E);
                lcd_draw_string(&dc, g->pxmin-fixed12.width/2-len*fixed8.width, py-fixed12.height/2, s);
            }
        }
        lcd_draw_segments(buf, n, BLACK);
        ...
        for (int i=0 ; i<n ; i++) {
            ...
            buf[2*i+1].x = g->pxmax;
            buf[2*i+1].y = py;
            buf[2*i].x = g->pxmax - (d!=1.0 ? SUBTICKSIZE : TICKSIZE);
            buf[2*i].y = py;
        }
        lcd_draw_segments(buf, n, BLACK);
    }
...
static int g_draw_plot(Graph *g, header *hdx, header *hdy)
{
    ...
    // For each matrix's  line
    for (int k = 0; k < ry; k++) {

        uint32_t offset;
        // Extra +5 space at the end to store extra data (color, etc)
        SPoint *buf = alloc_shmem_buffer(cx + 5, &offset);

        // Calculate X coordinates
        // If there's more than one x line, move pointer, else keep it
        real *current_x = (rx > 1) ? (x + k * cx) : x;

        if (!(g->flags & G_XLOG)) {
            for (int i = 0 ; i < cx ; i++) {
                buf[i].x = (short)(g->pxmin + (current_x[i] - g->xmin) * g->xfactor);
            }
        } else {
            for (int i = 0 ; i < cx ; i++) {
                buf[i].x = (short)(g->pxmin + log10(current_x[i] / g->xmin) * g->xfactor);
            }
        }

        // Calculate Y coordinates
        real *current_y = y + k * cy;

        if (!(g->flags & G_YLOG)) {
            for (int i = 0 ; i < cx ; i++) {
                if (current_y[i] > g->ymax)            /* avoid int16 overflow */
                    buf[i].y = g->pymin - 5;
                else if (current_y[i] < g->ymin)       /* avoid int16 overflow */
                    buf[i].y = g->pymax + 5;
                else                                   /* calculate standard case */
                    buf[i].y = (short)(g->pymin + (g->ymax - current_y[i]) * g->yfactor);
            }
        } else {
            for (int i = 0 ; i < cx ; i++) {
                buf[i].y = (short)(g->pymax - log10(current_y[i] / g->ymin) * g->yfactor);
            }
        }

        g_draw_path2d(g, buf, cx, g->color);
        // Update styles for next line
